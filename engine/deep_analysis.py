# /KotlinForge/engine/deep_analysis.py
# KotlinForge Deep Analysis Engine
...
class DeepAnalysisEngine:
    """
    Deep Analysis Engine v2 for KotlinForge

    Pipeline:
    - Chunk → Summarize → Meta-summarize → Reconstruct

    Focused on Kotlin structure:
    - packages, imports
    ...
    - idiomatic Kotlin patterns (val/var, null‑safety, when, smart casts)
...
# ============================= #
    ...
    def _log(self, stage: str, message: str, extra: Optional[Dict[str, Any]] = None):
        ...
        if self.debug:
            print(f"[KotlinDeepAnalysis:{stage}] {message}")
...
# =============================== #
    ...
    def summarize_chunk(self, chunk: str, index: int, total: int) -> str:
        topic = (
            "Summarize this Kotlin code for later reconstruction. "
            "Focus on package structure, imports, classes, objects, interfaces, "
            "data classes, sealed classes, top‑level functions, extension functions, "
            "coroutines, suspend functions, and logic flow:\n\n"
            f"{chunk}"
        )
    ...
    def merge_summaries(self, summaries: List[str]) -> str:
        ...
        topic = (
            "Merge these Kotlin code summaries into a single, coherent project summary. "
            "Describe package structure, imports, classes, objects, interfaces, "
            "data classes, sealed classes, top‑level functions, extension functions, "
            "coroutines, suspend functions, and overall architecture:\n\n"
            f"{joined}"
        )
    ...
    def analyze_from_summary(self, meta_summary: str) -> str:
        topic = (
            "Using ONLY this project summary, reconstruct or refactor the Kotlin code. "
            "Preserve logic, improve structure, and output ONLY Kotlin code. "
            "If multiple files or packages are implied, use // FILE: markers such as "
            "Main.kt, utils/Helpers.kt, or data/Models.kt:\n\n"
            f"{meta_summary}"
        )
...
# =============================== #
    ...
    def run(self, code: str) -> str:
        self.events.clear()
        self._log("run_start", "Kotlin Deep Analysis v2 run started.")
        ...
        self._log("run_complete", "Kotlin Deep Analysis v2 completed successfully.")
        return corrected
